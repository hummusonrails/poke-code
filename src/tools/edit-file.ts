...
function buildDiffPreview(filePath: string, oldContent: string, newContent: string): string {
  ...
  const changedLines = patch.split('\n').slice(4).filter(l => l.startsWith('+') || l.startsWith('-'));
...
export async function editFileTool(params: EditFileParams): Promise<string> {
  ...
  if (secondIdx !== -1) {
    throw new Error(`old_string is ambiguous in ${params.path} (found multiple occurrences). Use replace_all or provide a longer string.`);
  }
