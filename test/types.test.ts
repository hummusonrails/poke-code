...
import type { ToolCall, ParsedResponse, SessionEntry, PokeConfig, HandleInfo } from '../src/types.js';
import type { ConversationEvent, ToolEvent } from '../src/types.js';
...
describe('types', () => {
  ...
  it('SessionEntry supports all roles', () => {
    ...
    const assistant: SessionEntry = { role: 'assistant', content: 'hi', toolCalls: [], timestamp: '2026-01-01T00:00:01Z' };
    const tool: SessionEntry = { role: 'tool', results: [{ tool: 'bash', params: { command: 'ls' }, output: 'file.txt' }], timestamp: '2026-01-01T00:00:02Z' };
...
describe('ConversationEvent type', () => {
  ...
  it('accepts tool_result events', () => {
    const event: ConversationEvent = { type: 'tool_result', result: { tool: 'read_file', params: { path: '/tmp/x' }, output: 'content' } };
...
describe('ToolEvent type', () => {
  ...
  it('accepts result events', () => {
    const event: ToolEvent = { type: 'result', result: { tool: 'bash', params: { command: 'ls' }, output: 'file.txt' } };
