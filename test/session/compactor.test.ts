...
describe("compactHistory", () => {
  ...
  it("summarizes conversation with both user and assistant text", () => {
    ...
    expect(result.length).toBeLessThan(entries.map((e) => e.content ?? "").join("").length * 10);
  });

  it("truncates long assistant responses to 200 chars", () => {
    const longContent = "a".repeat(300);
    const entries: SessionEntry[] = [{ role: "assistant", content: longContent, timestamp: "2024-01-01T00:00:00Z" }];
    const result = compactHistory(entries);
  ...
  it("includes the summary header", () => {
    const entries: SessionEntry[] = [{ role: "user", content: "Hello", timestamp: "2024-01-01T00:00:00Z" }];
    const result = compactHistory(entries);
